    ...
    def __init__(self, in_ch, out_ch, g=16, channel_att=False, spatial_att=False):
        ...
        if channel_att:
            self.att_c = nn.Sequential(
                nn.Conv2d(2*out_ch, out_ch//g, 1, 1, 1), # 由0变为1
                nn.ReLU(),
                nn.Conv2d(out_ch//g, out_ch, 1, 1, 1),
                nn.Sigmoid()
            )
        if spatial_att:
...
class AMCC2(nn.Module):
    ...
    def forward(self, input):
        ...
        conv2 = self.conv2(F.avg_pool2d(conv1, kernel_size=3, stride=2,padding=0)) # ([18, 64, 96, 96]) -> ([18, 64, 48, 48]) -> ([18, 128, 48, 48]) kernel_size=2 变为 3 同时加入 padding=1
        conv3 = self.conv3(F.avg_pool2d(conv2, kernel_size=3, stride=2,padding=0)) # ([18, 128, 48, 48]) -> ([18, 64, 24, 24]) -> ([18, 256, 24, 24])
        conv4 = self.conv4(F.avg_pool2d(conv3, kernel_size=3, stride=2,padding=0)) # ([18, 256, 24, 24]) -> ([18, 64, 12, 12]) -> ([18, 512, 12, 12])
        conv5 = self.conv5(F.avg_pool2d(conv4,  kernel_size=3, stride=2,padding=0)) # ([18, 512, 12, 12]) -> ([18, 64, 6, 6]) -> ([18, 512, 6, 6])
