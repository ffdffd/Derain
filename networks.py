...
class AMCC2(nn.Module):
    ...
    def forward(self, input):
        ...
        return x, i
class AMCC2_wo_p2(nn.Module):
    def __init__(self, recurrent_iter=6, use_GPU=True, factor=10e-3, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
                 channel_att=False, spatial_att=False, upMode='bilinear', core_bias=False):
        super(AMCC2_wo_p2, self).__init__()
        #============================================ Layer 2 ===================================
        self.upMode = upMode
        self.burst_length = burst_length
        self.core_bias = core_bias
    ...
    def forward(self, input):
        ...
        # x_list = []
        input1 = torch.cat((input, pred1), 1) # 将input和core1进行拼接  得到input1 ([18, 6, 96, 96]) 
        input3 = torch.cat((input1, pred3), 1) # 将input2和core2进行拼接  得到input3 ([18, 12, 96, 96]) 注意，input堆叠多次后，self.conv0(x) 的输入通道也要相应改变
        # del input1,input2
        # torch.cuda.empty_cache()
        # torch.cuda.empty_cache()
        for i in range(self.iteration):  #　以下逐级扩张
        ...
        return x, i
class AMCC2_wo_p3(nn.Module):
    def __init__(self, recurrent_iter=6, use_GPU=True, factor=10e-3, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
                 channel_att=False, spatial_att=False, upMode='bilinear', core_bias=False):
        super(AMCC2_wo_p3, self).__init__()
        #============================================ Layer 2 ===================================
        self.upMode = upMode
        self.burst_length = burst_length
        self.core_bias = core_bias
    ...
    def forward(self, input):
        ...
        # x_list = []
        input1 = torch.cat((input, pred1), 1) # 将input和core1进行拼接  得到input1 ([18, 6, 96, 96]) 
        input3 = torch.cat((input1, pred2), 1) # 将input1和core2进行拼接  得到input2 ([18, 9, 96, 96]) 
        # del input1,input2
        # torch.cuda.empty_cache()
        # torch.cuda.empty_cache()
        for i in range(self.iteration):  #　以下逐级扩张
...
class AMCC2_17(nn.Module):
    def __init__(self, recurrent_iter=6, use_GPU=True, factor=10e-3, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
                 channel_att=False, spatial_att=False, upMode='bilinear', core_bias=False):
        super(AMCC2_17, self).__init__()
        #============================================ Layer 2 ===================================
        self.upMode = upMode
        self.burst_length = burst_length
        self.core_bias = core_bias
