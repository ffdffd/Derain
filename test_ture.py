...
parser.add_argument("--logdir", type=str, default="/data1/hjh/62190446236f408cbb1b3bb08c8b1241/JackFiles/ProjectData/RainData/DeRain", help='path to model and log files')  # 使用自己训练得到的模型
# parser.add_argument("--save_path", type=str, default="results/results_PReNet/", help='path to save results')
parser.add_argument("--data_path", type=str, default="/data1/hjh/62190446236f408cbb1b3bb08c8b1241/JackFiles/ProjectData/RainData/DeRain/Rain200L/Rain200L/test/rain", help='path to training data')
...
parser.add_argument("--save_path", type=str, default="/home/huangjiehui/Project/DerainNet/JackCode/Derain/Result/", help='path to save results')
...
def main():
    ...
    # Build model
    print('Loading model ...\n')
    model = AMCC2_17(opt.recurrent_iter, opt.use_GPU)
    print_network(model)
    if opt.use_GPU:
        model = model.cuda()
    model.load_state_dict(torch.load(os.path.join(opt.logdir, '200L_1016_net_latest.pth')))
    model.eval()
    ...
    for img_name in os.listdir(opt.data_path):
        if is_image(img_name):
            ...
            with torch.no_grad(): #
                ...
                #======================================查看测试损失 Start============================
                # input image
                gt_name = img_name.replace("x2","")
                gt_path = os.path.join(opt.data_path.replace("rain","norain"),gt_name )
